...
'''
sample_parser.py
Provides error counting functions for xml_testing.py
'''
...
class SampleParser:
    ...
    def diff_heap(self, sampleA, sampleB, numHeap):
        ...
        # a list of error counts for each global var in the heap
        return errCount, int(re.search(r'\d+', sampleB[0].text).group()), \
               int(re.search(r'\d+', sampleB[1].text).group())
